namespace MapPortingUtility
{
    ...
    public class T4xportHelper : ExportHelper
    {
        private const string EXECUTABLE_NAME = "CoDWaWmp.exe";

        public static Map[] GetMapList(ref Paths paths)
        {
            List<Map> maps = new List<Map>();
            string basePath = paths.T4Path;
            ...
            for (int i = 0; i < CANONICAL_LANGUAGES.Length; i++)
            {
                ...
                {
                    MessageBox.Show($"Could not list canonical maps for t4 for language {lang}!\n{ex}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
