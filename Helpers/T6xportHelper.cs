namespace MapPortingUtility
{
    ...
    public class T6xportHelper : ExportHelper
    {
        private const string EXECUTABLE_NAME = "t6mp.exe";

        public static Map[] GetMapList(ref Paths paths)
        {
            List<Map> maps = new List<Map>();
            string basePath = paths.T6Path;
            ...
            for (int i = 0; i < CANONICAL_LANGUAGES.Length; i++)
            {
                ...
                {
                    MessageBox.Show($"Could not list canonical maps for t6 for language {lang}!\n{ex}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            ...
            if (Directory.Exists(Path.Combine(basePath, USERMAPS_MAP_DIRECTORY))) {
                try {
                    ...
                    MessageBox.Show($"Could not list user maps for t6!\n{ex}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
